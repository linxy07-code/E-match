...
def render_company_past_transactions(db, user_id):
    ...
    # SAME LOGIC AS PERSONAL (use int-safe compare like your fixed version)
    seller_tx = [
    ...
    def render_card(t, role):
        ...
        # EXACT SAME STRUCTURE AS PERSONAL (only company fields added)
        ...
        ">
            ...
            {role}<br>
            👤 Buyer: {buyer_display}<br>
            🏪 Seller: {seller_display}<br>
            💰 {price_disp}<br>
            🆔 {t.get("item_id")}<br>
            🕒 {date_str}
        </div>
